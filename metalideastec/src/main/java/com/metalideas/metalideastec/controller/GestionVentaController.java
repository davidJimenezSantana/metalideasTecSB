...
public class GestionVentaController {
    ...
    @GetMapping(value = "/HacerRegistro")
    public String hacerRegistro(Model model) {
        Venta venta = new Venta();
        List<Producto> productos = new ArrayList<>();
        model.addAttribute("listaCarrito", productos);
        model.addAttribute("Productos", productoServ.listarProductos());
        model.addAttribute("ventas", ventaServ.listarVenta());
        model.addAttribute("estadosVenta", estadVenta.listarEstados());
        model.addAttribute("venta", venta);
        model.addAttribute("metodosPago", metodoPagoServ.listarMetodoPago());

        return "vista/ventas/carrito/carrito";
    }

    @PostMapping(value = "/registrarVenta")
    public String guardarRegistroVenta(@ModelAttribute("venta") Venta venta) {
